...
IQE_MARKER_EXPRESSION="${IQE_MARKER_EXPRESSION:-smoke}"
IQE_FILTER_EXPRESSION="${IQE_FILTER_EXPRESSION:-}"
IQE_ENV="${IQE_ENV:-clowder_smoke}"
IQE_PLUGINS="${IQE_PLUGINS:-puptoo}"
...
oc cp -n $NAMESPACE "$IQE_LOCAL_PLUGIN_PATH" "$POD:iqe-insights-upload-processor-plugin"

echo "Installing local IQE plugin in the pod..."
oc exec -n $NAMESPACE $POD -- bash -c "cd iqe-insights-upload-processor-plugin && pip install -i https://pypi.python.org/simple -e ."

echo "Verifying plugin installation..."
oc exec -n $NAMESPACE $POD -- iqe plugin list | grep -i puptoo
...
IQE_CMD="iqe tests plugin puptoo -vvv -m \"$IQE_MARKER_EXPRESSION\""
