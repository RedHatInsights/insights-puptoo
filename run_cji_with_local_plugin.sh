...
oc cp -n $NAMESPACE "$IQE_LOCAL_PLUGIN_PATH" "$POD:iqe-insights-upload-processor"

echo "Installing local IQE plugin in the pod..."
oc exec -n $NAMESPACE $POD -- bash -c "cd iqe-insights-upload-processor && pip install -i https://pypi.python.org/simple -e ."
