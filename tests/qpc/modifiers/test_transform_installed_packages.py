...
def test_remove_installed_packages():
    host = {
        "system_profile": {"installed_packages": ["pkg1", "pkg2", "pkg3"]},
        "tags": [],
    }
    transformed_obj = {"removed": [], "modified": [], "missing_data": []}
    with patch(
        "src.puptoo.modifiers.qpc.transform_installed_packages"
...
def test_modify_installed_packages():
    ...
    assert len(transformed_obj["removed"]) == 1
    assert (
        "installed_packages: prepending default epoch failure: "
        in transformed_obj["removed"][0]
    )
