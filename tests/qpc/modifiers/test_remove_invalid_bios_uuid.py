from src.puptoo.modifiers.qpc.remove_invalid_bios_uuid import RemoveInvalidBiosUUID
...
def test_remove_invalid_bios_uuid():
    ...
    assert (
        "invalid uuid: 45AA7104-5CB0-4A75-945D-7173C8DC5744443"
        in transformed_obj["removed"]
    )


def test_remove_invalid_bios_uuid_of_boolean_type():
    host = {"fqdn": "virt-who.example.com", "bios_uuid": True}
    transformed_obj = {"removed": []}
    RemoveInvalidBiosUUID().run(host, transformed_obj)
    assert host == {"fqdn": "virt-who.example.com"}
...
def test_remove_invalid_bios_uuid_of_number_type():
    host = {"fqdn": "virt-who.example.com", "bios_uuid": 100}
    transformed_obj = {"removed": []}
    RemoveInvalidBiosUUID().run(host, transformed_obj)
    assert host == {"fqdn": "virt-who.example.com"}
...
def test_remove_empty_bios_uuid():
    host = {"fqdn": "virt-who.example.com", "bios_uuid": ""}
    transformed_obj = {"removed": []}
    RemoveInvalidBiosUUID().run(host, transformed_obj)
    assert host == {"fqdn": "virt-who.example.com"}
