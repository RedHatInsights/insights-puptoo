...
uv --project iqe-insights-upload-processor-plugin sync --frozen
```

This installs all IQE dependencies and the local plugin in editable mode into
`iqe-insights-upload-processor-plugin/.venv/`.

### Activate IQE Environment
```bash
source iqe-insights-upload-processor-plugin/.venv/bin/activate
```
...
```bash
uv --project iqe-insights-upload-processor-plugin lock
uv --project iqe-insights-upload-processor-plugin sync
```

Then commit the updated lockfile:
```bash
git add iqe-insights-upload-processor-plugin/uv.lock
```
...
### Run Smoke Tests
```bash
ENV_FOR_DYNACONF=stage_proxy uv --project iqe-insights-upload-processor-plugin run \
...
export IQE_LOCAL_PLUGIN_PATH="$PWD/iqe-insights-upload-processor-plugin"
...
Check installation:
```bash
uv --project iqe-insights-upload-processor-plugin run iqe plugin list
```
...
# iqe-insights-upload-processor-plugin/.venv/bin/python       = IQE test environment
```

## Further Reading
- Full IQE plugin documentation: `iqe-insights-upload-processor-plugin/README.md`
